...
- `desktop/`: Tauri 2 app (`src/main.rs` commands `get_state`/`set_port`/`read_config`/`save_config`, static `ui/index.html` with `withGlobalTauri`, no Node build). Config path is `argv[1]` or `./client.toml` (a template is written if missing); state changes are pushed to the page as the `state` event. Running server and client on one host with same-number mappings makes the probe hit the server's own listener; use `expose_bind = "::1"` there.
